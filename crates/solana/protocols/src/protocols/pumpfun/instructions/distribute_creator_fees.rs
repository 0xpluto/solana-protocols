...
//! # Accounts
//!
//! Deliberately no account struct. The IDL declares 7; mainnet has been
...
impl crate::parsing::FromInstructionData for DistributeCreatorFeesParams {
    fn from_instruction_data(data: &[u8]) -> Result<Self, crate::parsing::InstructionParseError> {
        ...
        Err(
            crate::parsing::InstructionParseError::DeserializationFailed(format!(
                "DistributeCreatorFeesParams takes no arguments, got {} bytes",
                data.len()
            )),
        )
    }
