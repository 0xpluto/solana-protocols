...
/// `sha256("event:WithdrawEvent")[..8]`, derived at compile time.
pub const WITHDRAW_EVENT_DISCRIMINATOR: [u8; 8] =
    solana_protocols_macros::anchor_event_discriminator!("WithdrawEvent");
...
#[idl(program = "pump_amm", event = "WithdrawEvent")]
pub struct WithdrawEvent {
    /// `timestamp` — declared by the program IDL.
    pub timestamp: i64,
    /// `lp_token_amount_in` — declared by the program IDL.
...
impl ProtocolEvent for WithdrawEvent {
    const DISCRIMINATOR: [u8; 8] = WITHDRAW_EVENT_DISCRIMINATOR;
    const NAME: &'static str = "WithdrawEvent";
}
...
mod tests {
    ...
    fn the_real_body_decodes_and_refuses_a_trailing_byte() {
        let hex = include_str!("../../../../fixtures/pumpswap/event_bodies/WithdrawEvent_248.hex");
        ...
        let ev = WithdrawEvent::from_event_body(&body).expect("real body decodes");

        let mut longer = body.clone();
        longer.push(0);
        assert!(
            WithdrawEvent::from_event_body(&longer).is_err(),
