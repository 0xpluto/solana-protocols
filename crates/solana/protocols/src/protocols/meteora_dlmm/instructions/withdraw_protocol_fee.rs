//! `withdraw_protocol_fee` instruction.
//!
//! Generated wrapper. Re-run `tools/gen_dlmm_ix.py` against the
//! `meteora-dlmm-sdk` registry source to regenerate after an SDK
//! upgrade — this file isn't hand-edited.
//!
//! - **Parse:** [`WithdrawProtocolFeeIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`WithdrawProtocolFeeIx::build`] returns a ready-to-submit
//!   `solana_sdk::instruction::Instruction`. Account writability /
//!   signer flags come from the `#[account(...)]` attrs which mirror
//!   the SDK's `instruction_with_remaining_accounts` exactly.

use solana_program::pubkey::Pubkey;
use solana_protocols_macros::AccountMetas;

use crate::parsing::{InstructionParseError, ParsedInstruction};

use super::parse_anchor_args;

pub use meteora_dlmm_sdk::instructions::WithdrawProtocolFeeInstructionArgs;
pub use meteora_dlmm_sdk::instructions::WITHDRAW_PROTOCOL_FEE_DISCRIMINATOR;

#[derive(Debug, Clone, AccountMetas)]
pub struct WithdrawProtocolFeeAccounts {
    #[account(writable)]
    pub lb_pair: Pubkey,
    #[account(writable)]
    pub reserve_x: Pubkey,
    #[account(writable)]
    pub reserve_y: Pubkey,
    #[account]
    pub token_x_mint: Pubkey,
    #[account]
    pub token_y_mint: Pubkey,
    #[account(writable)]
    pub receiver_token_x: Pubkey,
    #[account(writable)]
    pub receiver_token_y: Pubkey,
    #[account]
    pub claim_fee_operator: Pubkey,
    #[account(signer)]
    pub operator: Pubkey,
    #[account]
    pub token_x_program: Pubkey,
    #[account]
    pub token_y_program: Pubkey,
    #[account]
    pub memo_program: Pubkey,
}

#[derive(Debug, Clone)]
pub struct WithdrawProtocolFeeIx {
    pub accounts: WithdrawProtocolFeeAccounts,
    pub args: WithdrawProtocolFeeInstructionArgs,
}

impl WithdrawProtocolFeeIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`WITHDRAW_PROTOCOL_FEE_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = WithdrawProtocolFeeAccounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<WithdrawProtocolFeeInstructionArgs>(
            &ix.data,
            &WITHDRAW_PROTOCOL_FEE_DISCRIMINATOR,
            "withdraw_protocol_fee",
        )?;
        Ok(Self { accounts, args })
    }

    /// Build a ready-to-submit instruction from accounts (+ args
    /// when the ix takes any). Account writable / signer flags come
    /// from the [`AccountMetas`] derive on [`WithdrawProtocolFeeAccounts`].
    /// The data section is the 8-byte discriminator followed by
    /// borsh-serialised args.
    pub fn build(
        accounts: WithdrawProtocolFeeAccounts,
        args: WithdrawProtocolFeeInstructionArgs,
    ) -> ::solana_sdk::instruction::Instruction {
        Self { accounts, args }.to_instruction()
    }

    /// Variant of [`Self::build`] for callers that already hold a fully
    /// populated [`WithdrawProtocolFeeIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = WITHDRAW_PROTOCOL_FEE_DISCRIMINATOR.to_vec();
        let args_bytes =
            ::borsh::to_vec(&self.args).expect("borsh serialize withdraw_protocol_fee args");
        data.extend_from_slice(&args_bytes);
        ::solana_sdk::instruction::Instruction {
            program_id: super::super::PROGRAM_ID,
            accounts: self.accounts.to_account_metas(),
            data,
        }
    }
}
