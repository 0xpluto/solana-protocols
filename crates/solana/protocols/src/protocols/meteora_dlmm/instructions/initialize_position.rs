...
//!
//! - **Parse:** [`InitializePositionIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`InitializePositionIx::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::InitializePositionInstructionArgs;
pub use meteora_dlmm_sdk::instructions::INITIALIZE_POSITION_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct InitializePositionIx {
    pub accounts: InitializePositionAccounts,
    pub args: InitializePositionInstructionArgs,
}

impl InitializePositionIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`INITIALIZE_POSITION_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = InitializePositionAccounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<InitializePositionInstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`InitializePositionAccounts`].
    ...
    /// populated [`InitializePositionIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = INITIALIZE_POSITION_DISCRIMINATOR.to_vec();
        let args_bytes =
            ::borsh::to_vec(&self.args).expect("borsh serialize initialize_position args");
