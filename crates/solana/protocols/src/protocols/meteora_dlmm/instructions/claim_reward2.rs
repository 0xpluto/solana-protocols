...
//!
//! - **Parse:** [`ClaimReward2Ix::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`ClaimReward2Ix::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::ClaimReward2InstructionArgs;
pub use meteora_dlmm_sdk::instructions::CLAIM_REWARD2_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct ClaimReward2Ix {
    pub accounts: ClaimReward2Accounts,
    pub args: ClaimReward2InstructionArgs,
}

impl ClaimReward2Ix {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`CLAIM_REWARD2_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = ClaimReward2Accounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<ClaimReward2InstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`ClaimReward2Accounts`].
    ...
    /// populated [`ClaimReward2Ix`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = CLAIM_REWARD2_DISCRIMINATOR.to_vec();
        let args_bytes = ::borsh::to_vec(&self.args).expect("borsh serialize claim_reward2 args");
