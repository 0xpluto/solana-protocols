//! `initialize_preset_parameter` instruction.
//!
...
//!
//! - **Parse:** [`InitializePresetParameterIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`InitializePresetParameterIx::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::InitializePresetParameterInstructionArgs;
pub use meteora_dlmm_sdk::instructions::INITIALIZE_PRESET_PARAMETER_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct InitializePresetParameterIx {
    pub accounts: InitializePresetParameterAccounts,
    pub args: InitializePresetParameterInstructionArgs,
}

impl InitializePresetParameterIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`INITIALIZE_PRESET_PARAMETER_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = InitializePresetParameterAccounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<InitializePresetParameterInstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`InitializePresetParameterAccounts`].
    ...
    /// populated [`InitializePresetParameterIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = INITIALIZE_PRESET_PARAMETER_DISCRIMINATOR.to_vec();
        let args_bytes =
            ::borsh::to_vec(&self.args).expect("borsh serialize initialize_preset_parameter args");
