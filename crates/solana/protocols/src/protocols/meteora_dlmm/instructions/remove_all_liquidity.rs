//! `remove_all_liquidity` instruction.
//!
...
//!
//! - **Parse:** [`RemoveAllLiquidityIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`RemoveAllLiquidityIx::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::REMOVE_ALL_LIQUIDITY_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct RemoveAllLiquidityIx {
    pub accounts: RemoveAllLiquidityAccounts,
}

impl RemoveAllLiquidityIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`REMOVE_ALL_LIQUIDITY_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = RemoveAllLiquidityAccounts::from_pubkeys(&ix.accounts)?;
        Ok(Self { accounts })
    }

    /// Build a ready-to-submit instruction from accounts (+ args
    /// when the ix takes any). Account writable / signer flags come
    /// from the [`AccountMetas`] derive on [`RemoveAllLiquidityAccounts`].
    /// The data section is the 8-byte discriminator followed by
    /// borsh-serialised args.
    pub fn build(accounts: RemoveAllLiquidityAccounts) -> ::solana_sdk::instruction::Instruction {
        Self { accounts }.to_instruction()
    }

    /// Variant of [`Self::build`] for callers that already hold a fully
    /// populated [`RemoveAllLiquidityIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let data = REMOVE_ALL_LIQUIDITY_DISCRIMINATOR.to_vec();
