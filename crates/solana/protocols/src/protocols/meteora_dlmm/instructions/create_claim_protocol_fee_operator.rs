//! `create_claim_protocol_fee_operator` instruction.
//!
...
//!
//! - **Parse:** [`CreateClaimProtocolFeeOperatorIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`CreateClaimProtocolFeeOperatorIx::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::CREATE_CLAIM_PROTOCOL_FEE_OPERATOR_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct CreateClaimProtocolFeeOperatorIx {
    pub accounts: CreateClaimProtocolFeeOperatorAccounts,
}

impl CreateClaimProtocolFeeOperatorIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`CREATE_CLAIM_PROTOCOL_FEE_OPERATOR_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = CreateClaimProtocolFeeOperatorAccounts::from_pubkeys(&ix.accounts)?;
        Ok(Self { accounts })
    }

    /// Build a ready-to-submit instruction from accounts (+ args
    /// when the ix takes any). Account writable / signer flags come
    /// from the [`AccountMetas`] derive on [`CreateClaimProtocolFeeOperatorAccounts`].
    ...
    /// populated [`CreateClaimProtocolFeeOperatorIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let data = CREATE_CLAIM_PROTOCOL_FEE_OPERATOR_DISCRIMINATOR.to_vec();
