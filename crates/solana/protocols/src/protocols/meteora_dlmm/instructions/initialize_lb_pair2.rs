//! `initialize_lb_pair2` instruction.
//!
...
//!
//! - **Parse:** [`InitializeLbPair2Ix::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`InitializeLbPair2Ix::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::InitializeLbPair2InstructionArgs;
pub use meteora_dlmm_sdk::instructions::INITIALIZE_LB_PAIR2_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct InitializeLbPair2Ix {
    pub accounts: InitializeLbPair2Accounts,
    pub args: InitializeLbPair2InstructionArgs,
}

impl InitializeLbPair2Ix {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`INITIALIZE_LB_PAIR2_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = InitializeLbPair2Accounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<InitializeLbPair2InstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`InitializeLbPair2Accounts`].
    ...
    /// populated [`InitializeLbPair2Ix`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = INITIALIZE_LB_PAIR2_DISCRIMINATOR.to_vec();
        let args_bytes =
            ::borsh::to_vec(&self.args).expect("borsh serialize initialize_lb_pair2 args");
