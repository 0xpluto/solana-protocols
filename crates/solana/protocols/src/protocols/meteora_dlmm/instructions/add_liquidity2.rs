...
//!
//! - **Parse:** [`AddLiquidity2Ix::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`AddLiquidity2Ix::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::AddLiquidity2InstructionArgs;
pub use meteora_dlmm_sdk::instructions::ADD_LIQUIDITY2_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct AddLiquidity2Ix {
    pub accounts: AddLiquidity2Accounts,
    pub args: AddLiquidity2InstructionArgs,
}

impl AddLiquidity2Ix {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`ADD_LIQUIDITY2_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = AddLiquidity2Accounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<AddLiquidity2InstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`AddLiquidity2Accounts`].
    ...
    /// populated [`AddLiquidity2Ix`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = ADD_LIQUIDITY2_DISCRIMINATOR.to_vec();
        let args_bytes = ::borsh::to_vec(&self.args).expect("borsh serialize add_liquidity2 args");
