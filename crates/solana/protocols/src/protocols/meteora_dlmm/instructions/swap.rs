...
//!
//! - **Parse:** [`SwapIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`SwapIx::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::SwapInstructionArgs;
pub use meteora_dlmm_sdk::instructions::SWAP_DISCRIMINATOR;
...
pub struct SwapIx {
    ...
    pub args: SwapInstructionArgs,
}

impl SwapIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`SWAP_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = SwapAccounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<SwapInstructionArgs>(&ix.data, &SWAP_DISCRIMINATOR, "swap")?;
    ...
    /// from the [`AccountMetas`] derive on [`SwapAccounts`].
    ...
    /// populated [`SwapIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = SWAP_DISCRIMINATOR.to_vec();
        let args_bytes = ::borsh::to_vec(&self.args).expect("borsh serialize swap args");
