//! `set_pair_status_permissionless` instruction.
//!
...
//!
//! - **Parse:** [`SetPairStatusPermissionlessIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`SetPairStatusPermissionlessIx::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::SetPairStatusPermissionlessInstructionArgs;
...
#[derive(Debug, Clone)]
pub struct SetPairStatusPermissionlessIx {
    pub accounts: SetPairStatusPermissionlessAccounts,
    pub args: SetPairStatusPermissionlessInstructionArgs,
}

impl SetPairStatusPermissionlessIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`SET_PAIR_STATUS_PERMISSIONLESS_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = SetPairStatusPermissionlessAccounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<SetPairStatusPermissionlessInstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`SetPairStatusPermissionlessAccounts`].
    ...
    /// populated [`SetPairStatusPermissionlessIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = SET_PAIR_STATUS_PERMISSIONLESS_DISCRIMINATOR.to_vec();
        let args_bytes = ::borsh::to_vec(&self.args)
            .expect("borsh serialize set_pair_status_permissionless args");
