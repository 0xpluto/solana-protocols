//! `add_liquidity_one_side_precise2` instruction.
//!
...
//!
//! - **Parse:** [`AddLiquidityOneSidePrecise2Ix::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`AddLiquidityOneSidePrecise2Ix::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::AddLiquidityOneSidePrecise2InstructionArgs;
pub use meteora_dlmm_sdk::instructions::ADD_LIQUIDITY_ONE_SIDE_PRECISE2_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct AddLiquidityOneSidePrecise2Ix {
    pub accounts: AddLiquidityOneSidePrecise2Accounts,
    pub args: AddLiquidityOneSidePrecise2InstructionArgs,
}

impl AddLiquidityOneSidePrecise2Ix {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`ADD_LIQUIDITY_ONE_SIDE_PRECISE2_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = AddLiquidityOneSidePrecise2Accounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<AddLiquidityOneSidePrecise2InstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`AddLiquidityOneSidePrecise2Accounts`].
    ...
    /// populated [`AddLiquidityOneSidePrecise2Ix`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = ADD_LIQUIDITY_ONE_SIDE_PRECISE2_DISCRIMINATOR.to_vec();
        let args_bytes = ::borsh::to_vec(&self.args)
            .expect("borsh serialize add_liquidity_one_side_precise2 args");
