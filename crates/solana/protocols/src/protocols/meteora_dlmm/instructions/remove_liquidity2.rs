...
//!
//! - **Parse:** [`RemoveLiquidity2Ix::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`RemoveLiquidity2Ix::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::RemoveLiquidity2InstructionArgs;
pub use meteora_dlmm_sdk::instructions::REMOVE_LIQUIDITY2_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct RemoveLiquidity2Ix {
    pub accounts: RemoveLiquidity2Accounts,
    pub args: RemoveLiquidity2InstructionArgs,
}

impl RemoveLiquidity2Ix {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`REMOVE_LIQUIDITY2_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = RemoveLiquidity2Accounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<RemoveLiquidity2InstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`RemoveLiquidity2Accounts`].
    ...
    /// populated [`RemoveLiquidity2Ix`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = REMOVE_LIQUIDITY2_DISCRIMINATOR.to_vec();
        let args_bytes =
            ::borsh::to_vec(&self.args).expect("borsh serialize remove_liquidity2 args");
