//! `update_dynamic_fee_parameters` instruction.
//!
...
//!
//! - **Parse:** [`UpdateDynamicFeeParametersIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`UpdateDynamicFeeParametersIx::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::UpdateDynamicFeeParametersInstructionArgs;
pub use meteora_dlmm_sdk::instructions::UPDATE_DYNAMIC_FEE_PARAMETERS_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct UpdateDynamicFeeParametersIx {
    pub accounts: UpdateDynamicFeeParametersAccounts,
    pub args: UpdateDynamicFeeParametersInstructionArgs,
}

impl UpdateDynamicFeeParametersIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`UPDATE_DYNAMIC_FEE_PARAMETERS_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = UpdateDynamicFeeParametersAccounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<UpdateDynamicFeeParametersInstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`UpdateDynamicFeeParametersAccounts`].
    ...
    /// populated [`UpdateDynamicFeeParametersIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = UPDATE_DYNAMIC_FEE_PARAMETERS_DISCRIMINATOR.to_vec();
        let args_bytes = ::borsh::to_vec(&self.args)
            .expect("borsh serialize update_dynamic_fee_parameters args");
