//! `swap_with_price_impact` instruction.
//!
...
//!
//! - **Parse:** [`SwapWithPriceImpactIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`SwapWithPriceImpactIx::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::SwapWithPriceImpactInstructionArgs;
pub use meteora_dlmm_sdk::instructions::SWAP_WITH_PRICE_IMPACT_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct SwapWithPriceImpactIx {
    pub accounts: SwapWithPriceImpactAccounts,
    pub args: SwapWithPriceImpactInstructionArgs,
}

impl SwapWithPriceImpactIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`SWAP_WITH_PRICE_IMPACT_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = SwapWithPriceImpactAccounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<SwapWithPriceImpactInstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`SwapWithPriceImpactAccounts`].
    ...
    /// populated [`SwapWithPriceImpactIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = SWAP_WITH_PRICE_IMPACT_DISCRIMINATOR.to_vec();
        let args_bytes =
            ::borsh::to_vec(&self.args).expect("borsh serialize swap_with_price_impact args");
