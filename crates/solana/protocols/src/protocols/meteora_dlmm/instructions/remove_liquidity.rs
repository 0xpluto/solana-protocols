...
//!
//! - **Parse:** [`RemoveLiquidityIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`RemoveLiquidityIx::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::RemoveLiquidityInstructionArgs;
pub use meteora_dlmm_sdk::instructions::REMOVE_LIQUIDITY_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct RemoveLiquidityIx {
    pub accounts: RemoveLiquidityAccounts,
    pub args: RemoveLiquidityInstructionArgs,
}

impl RemoveLiquidityIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`REMOVE_LIQUIDITY_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = RemoveLiquidityAccounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<RemoveLiquidityInstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`RemoveLiquidityAccounts`].
    ...
    /// populated [`RemoveLiquidityIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = REMOVE_LIQUIDITY_DISCRIMINATOR.to_vec();
        let args_bytes =
            ::borsh::to_vec(&self.args).expect("borsh serialize remove_liquidity args");
