//! `remove_liquidity_by_range` instruction.
//!
...
//!
//! - **Parse:** [`RemoveLiquidityByRangeIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`RemoveLiquidityByRangeIx::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::RemoveLiquidityByRangeInstructionArgs;
pub use meteora_dlmm_sdk::instructions::REMOVE_LIQUIDITY_BY_RANGE_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct RemoveLiquidityByRangeIx {
    pub accounts: RemoveLiquidityByRangeAccounts,
    pub args: RemoveLiquidityByRangeInstructionArgs,
}

impl RemoveLiquidityByRangeIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`REMOVE_LIQUIDITY_BY_RANGE_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = RemoveLiquidityByRangeAccounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<RemoveLiquidityByRangeInstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`RemoveLiquidityByRangeAccounts`].
    ...
    /// populated [`RemoveLiquidityByRangeIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = REMOVE_LIQUIDITY_BY_RANGE_DISCRIMINATOR.to_vec();
        let args_bytes =
            ::borsh::to_vec(&self.args).expect("borsh serialize remove_liquidity_by_range args");
