//! `add_liquidity_by_strategy_one_side` instruction.
//!
...
//!
//! - **Parse:** [`AddLiquidityByStrategyOneSideIx::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`AddLiquidityByStrategyOneSideIx::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::AddLiquidityByStrategyOneSideInstructionArgs;
pub use meteora_dlmm_sdk::instructions::ADD_LIQUIDITY_BY_STRATEGY_ONE_SIDE_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct AddLiquidityByStrategyOneSideIx {
    pub accounts: AddLiquidityByStrategyOneSideAccounts,
    pub args: AddLiquidityByStrategyOneSideInstructionArgs,
}

impl AddLiquidityByStrategyOneSideIx {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`ADD_LIQUIDITY_BY_STRATEGY_ONE_SIDE_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = AddLiquidityByStrategyOneSideAccounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<AddLiquidityByStrategyOneSideInstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`AddLiquidityByStrategyOneSideAccounts`].
    ...
    /// populated [`AddLiquidityByStrategyOneSideIx`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = ADD_LIQUIDITY_BY_STRATEGY_ONE_SIDE_DISCRIMINATOR.to_vec();
        let args_bytes = ::borsh::to_vec(&self.args)
            .expect("borsh serialize add_liquidity_by_strategy_one_side args");
