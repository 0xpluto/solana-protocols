...
//!
//! - **Parse:** [`Swap2Ix::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`Swap2Ix::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::Swap2InstructionArgs;
pub use meteora_dlmm_sdk::instructions::SWAP2_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct Swap2Ix {
    pub accounts: Swap2Accounts,
    pub args: Swap2InstructionArgs,
}

impl Swap2Ix {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`SWAP2_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = Swap2Accounts::from_pubkeys(&ix.accounts)?;
        let args =
            parse_anchor_args::<Swap2InstructionArgs>(&ix.data, &SWAP2_DISCRIMINATOR, "swap2")?;
    ...
    /// from the [`AccountMetas`] derive on [`Swap2Accounts`].
    ...
    /// populated [`Swap2Ix`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = SWAP2_DISCRIMINATOR.to_vec();
        let args_bytes = ::borsh::to_vec(&self.args).expect("borsh serialize swap2 args");
