...
//!
//! - **Parse:** [`SwapExactOut2Ix::parse`] decodes from a
//!   [`ParsedInstruction`] into typed accounts + args (when present).
//! - **Build:** [`SwapExactOut2Ix::build`] returns a ready-to-submit
...
pub use meteora_dlmm_sdk::instructions::SwapExactOut2InstructionArgs;
pub use meteora_dlmm_sdk::instructions::SWAP_EXACT_OUT2_DISCRIMINATOR;
...
#[derive(Debug, Clone)]
pub struct SwapExactOut2Ix {
    pub accounts: SwapExactOut2Accounts,
    pub args: SwapExactOut2InstructionArgs,
}

impl SwapExactOut2Ix {
    /// Parse a [`ParsedInstruction`] whose program id is the DLMM
    /// program and whose discriminator matches [`SWAP_EXACT_OUT2_DISCRIMINATOR`].
    pub fn parse(ix: &ParsedInstruction) -> Result<Self, InstructionParseError> {
        let accounts = SwapExactOut2Accounts::from_pubkeys(&ix.accounts)?;
        let args = parse_anchor_args::<SwapExactOut2InstructionArgs>(
            &ix.data,
    ...
    /// from the [`AccountMetas`] derive on [`SwapExactOut2Accounts`].
    ...
    /// populated [`SwapExactOut2Ix`] (e.g. just-parsed and being
    /// re-built for replay).
    pub fn to_instruction(&self) -> ::solana_sdk::instruction::Instruction {
        let mut data = SWAP_EXACT_OUT2_DISCRIMINATOR.to_vec();
        let args_bytes = ::borsh::to_vec(&self.args).expect("borsh serialize swap_exact_out2 args");
