//! `U128.128`-style fixed-point helpers. Verbatim port from the
//! reference `u128x128_math.rs`. All ops widen to `U256` to avoid
//! overflow on the multiply-then-divide pattern.

use ruint::aliases::U256;

/// Rounding direction for `mul_div` / `mul_shr` / `shl_div`.
...
/// `(x * y) / denominator`, returning `None` on overflow or division
/// by zero.
