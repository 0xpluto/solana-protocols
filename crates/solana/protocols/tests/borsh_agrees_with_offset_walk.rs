...
macro_rules! agree {
    ($ty:ty, $fixture:literal) => {{
        ...
        let walked = <$ty>::from_instruction_data(&data)
...
fn pumpfun_sell_decodes_the_same_both_ways() {
    agree!(solana_protocols::pumpfun::SellParams, "pumpfun/ix_sell.json");
}

#[test]
fn pumpswap_buy_decodes_the_same_both_ways() {
    agree!(solana_protocols::pumpswap::BuyParams, "pumpswap/ix_buy.json");
}

#[test]
fn pumpswap_sell_decodes_the_same_both_ways() {
    agree!(solana_protocols::pumpswap::SellParams, "pumpswap/ix_sell.json");
}
